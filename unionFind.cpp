...
static void arrayMerge(int** dest_arr, int* dest_keys, int** arr1, int* keys1, int size1, int** arr2, int* keys2, int size2, int scale, int* actual_size)
{
    dest_keys[0] = 0;
...
void UnionFind::unite(int key1, int key2)
{
    ...
    int* smaller_group_keys = new int[smaller_tree_size + 1]();
    int* bigger_group_keys = new int[bigger_tree_size + 1]();

    int final_size = bigger_tree_size + smaller_tree_size + 1;
    ...
    int actual_size = final_size - 1;
    arrayMerge(both_groups_arrays, both_groups_keys, bigger_group_arrays, bigger_group_keys, bigger_tree_size,
               smaller_group_arrays, smaller_group_keys, smaller_tree_size, scale, &actual_size);

    bigger_group->level_tree->buildAndFillTree(both_groups_keys, both_groups_arrays, actual_size);
